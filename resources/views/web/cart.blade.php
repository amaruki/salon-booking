        ...
        <div class="container mx-auto px-4 md:w-11/12">
            ...
            @if (session('unavailable_time_slots'))
                <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative" role="alert">
                    ...
                    <span class="block sm:inline">The following time slots are no longer available. Please remove them
                        from your cart to continue.</span>
                    <ul class="mt-2 list-disc list-inside text-sm text-red-600">


                        @foreach (session('unavailable_time_slots') as $unavailable_time_slot)
                            <li>{{ $unavailable_time_slot['date'] }}:
                                {{ date('g:i a', strtotime($unavailable_time_slot['start_time'])) }} -
                                {{ date('g:i a', strtotime($unavailable_time_slot['end_time'])) }}</li>
                        @endforeach
                    </ul>
                </div>
            @endif
