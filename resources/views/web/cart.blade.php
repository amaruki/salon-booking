    ...
    <div class="bg-gray-100 py-8" x-data="{ showCheckoutConfirmation: false }">
        <div class="container mx-auto px-4 md:w-11/12">
            ...
            @if (session('unavailable_time_slots'))
                <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative" role="alert">
                    ...
                    <span class="block sm:inline">{{ __('Slot waktu berikut sudah tidak tersedia. Silakan hapus dari keranjang Anda untuk melanjutkan.') }}</span>
                        ...
                        <table class="w-full">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-left font-semibold">{{ __('Tanggal') }}</th>
                                    <th class="text-left font-semibold">{{ __('Slot Waktu') }}</th>
                            ...
                            <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <form
                                                ...
                                                method="post"> @csrf @method('delete') <td class="py-4">
                                                    <button type="submit"
                                                        class="text-red-500 hover:text-red-600 font-semibold">{{ __('Hapus') }}</button>
                                                </td>
                                            </form>
                                        </tr>
                                    @endforeach
                                @else
                                    <tr>
                                        <td colspan="5" class="text-center pt-8">{{ __('Tidak ada item di keranjang') }}</td>
                                    </tr>
                                @endif
                            </tbody>
                        </table>
                ...
                <div class="md:w-1/4">
                    <div class="bg-white rounded-lg shadow-md p-6">
                        ...
                        <div class="flex justify-between mb-2">
                            <span>{{ __('Subtotal') }}</span> <span>Rp.{{ number_format($cart?->total, 2, '.', ',') }}</span>
                        </div>
                        <hr class="my-2">
                        <div class="flex justify-between mb-2">
        ...
        <div x-show="showCheckoutConfirmation" x-cloak
            ...
            <div class="bg-white rounded-lg p-4 max-w-md mx-auto" @click.outside="showCheckoutConfirmation = false">
                <h2 class="text-xl font-semibold">{{ __('Konfirmasi Checkout') }}</h2>
                <p>{{ __('Apakah Anda yakin ingin melakukan checkout?') }}</p>
