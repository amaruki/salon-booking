    ...
    <div class="md:w-9/12 w-full mx-auto">
        <div class="relative bg-white rounded-lg shadow-xl overflow-hidden my-8">
            ...
            <div class="bg-gray-50 px-6 py-3 text-sm">
                ...
                <a href="/services" class="text-gray-500 hover:text-pink-500">{{ __('Layanan') }}</a> &raquo;
            ...
            @if (Auth::user()?->role_id == 1 || Auth::user()?->role_id == 2)
                <div class="border-t border-gray-200 mt-8 px-6 py-6 bg-gray-50">
                    <h2 class="text-xl font-bold text-gray-800 mb-4">{{ __('Wawasan Analitik') }}</h2>
                    
                    <!-- Summary Stats -->
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                        <div class="bg-white p-4 rounded-lg shadow">
                            <span class="text-sm text-gray-500">{{ __('Total Dilihat') }}</span>
                            ...
                            <span class="text-xs text-gray-500">{{ __('Minggu Lalu:') }} {{ $viewsLastWeek }}</span>
                        </div>

                        <div class="bg-white p-4 rounded-lg shadow">
                            <span class="text-sm text-gray-500">{{ __('Total Janji Temu') }}</span>
                            <div class="flex items-center justify-between mt-1">
                            ...
                            <span class="text-xs text-gray-500">{{ __('Minggu Lalu:') }} {{ $appointmentsLastWeek }}</span>
                        </div>

                        <div class="bg-white p-4 rounded-lg shadow">
                            <span class="text-sm text-gray-500">{{ __('Janji Temu (Bulanan)') }}</span>
                            <div class="flex items-center justify-between mt-1">
                            ...
                            <span class="text-xs text-gray-500">{{ __('Perubahan Bulanan') }}</span>
                        </div>

                        <div class="bg-white p-4 rounded-lg shadow">
                            <span class="text-sm text-gray-500">{{ __('Total Pendapatan') }}</span>
                            <div class="flex items-center justify-between mt-1">
                            ...
                            <span class="text-xs text-gray-500">{{ __('Minggu Lalu:') }} Rp {{ number_format($totalRevenueLastWeek, 0, '.', '.') }}</span>
                        </div>
                    </div>
                    ...
                    <div x-data="{ activeAnalytic: 'weekly' }">
                        ...
                        <div x-show="activeAnalytic === 'weekly'" class="bg-white rounded-lg shadow overflow-hidden">
                            <div class="px-4 py-3 bg-gray-50 border-b">
                                <h3 class="font-medium">{{ __('Slot Waktu Terpopuler Minggu Lalu') }}</h3>
                            </div>
                            <div class="overflow-x-auto">
                                <table class="min-w-full divide-y divide-gray-200">
                                    <thead class="bg-gray-50">
                                        <tr>
                                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('Slot Waktu') }}</th>
                                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('Jumlah') }}</th>
                                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('Persentase') }}</th>
                                        </tr>
                                    </thead>
                        ...
                        <div x-show="activeAnalytic === 'allTime'" class="bg-white rounded-lg shadow overflow-hidden">
                            <div class="px-4 py-3 bg-gray-50 border-b">
                                <h3 class="font-medium">{{ __('Slot Waktu Terpopuler Sepanjang Waktu') }}</h3>
                            </div>
                            <div class="overflow-x-auto">
                                <table class="min-w-full divide-y divide-gray-200">
                                    <thead class="bg-gray-50">
                                    ...
                                    <tbody class="bg-white divide-y divide-gray-200">
                                        @php
                                            $totalAllTime = array_sum(array_column($timeSlotsStats->toArray(), 'count'));
                                        @endphp
